    ...
    <div className="space-y-3">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-muted/50 text-left">
              {["ФИО", "Email", "Телефон", "Оплата", "Забронир.", "Дата оплаты", "Канал", "Чек", ""].map((h) => (
                <th key={h} className="px-3 py-2 text-xs font-semibold text-muted-foreground whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              <tr><td colSpan={9} className="px-3 py-4 text-center text-muted-foreground text-sm">Загрузка...</td></tr>
            ) : participants.length === 0 ? (
              <tr><td colSpan={9} className="px-3 py-4 text-center text-muted-foreground text-sm">Нет участников</td></tr>
            ) : (
              participants.map((p) => <ParticipantRow key={p.id} participant={p} groupId={group.id} />)
            )}
          </tbody>
        </table>
      </div>
      ...
      {showAdd ? (
        <div className="flex gap-2 items-center">
          ...
          />
          <Button size="sm" onClick={handleAdd} disabled={create.isPending}>Добавить</Button>
